...
void EditorLayer::OnUpdate()
{
	...
	if (const RenderTargetDesc& desc = m_renderingSystem->GetRenderTarget()->GetDesc();
		m_sceneViewportWidth > 0.0f && m_sceneViewportHeight > 0.0f &&
		(desc.width != m_sceneViewportWidth || desc.height != m_sceneViewportHeight))
	{
		m_renderingSystem->SetRenderTargetSize(m_sceneViewportWidth, m_sceneViewportHeight);
...
void EditorLayer::OnImGuiRender()
{
	...
	D3D11_VIEWPORT vp = {};
	vp.Width = (float)app.GetWindow().GetWidth();
	vp.Height = (float)app.GetWindow().GetHeight();
	vp.TopLeftX = 0.0f;
	vp.TopLeftY = 0.0f;
	vp.MinDepth = 0.0f;
	vp.MaxDepth = 1.0f;
	...
	ImGui::Image(m_renderingSystem->GetRenderTarget()->GetView(), { m_sceneViewportWidth, m_sceneViewportHeight });
	ImGui::End();	
	ImGui::PopStyleVar();
